...
isgood:=function(w)local res,v,I,w,W;
  W:=w.operations.W();
  res:=[];
  if w.pw0>0 then Add(res,[[1..W.dim],w.pw0]);fi;
  for v in w.elm do
   I:=Set(WeylWordPerm(W,v));
   if LeftRefs(W,v)<>I then return false;fi;
...
#
  ...
  W:=Weyl(HW.cartan);
  ti:=CharTable(W);
  eig:=eigw(W,W.B(ti.classtext[ind]));
  eig:=reduce(eig,gal);
